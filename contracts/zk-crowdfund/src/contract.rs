...
fn threshold_check_complete(
    ...
    if output_variables.len() >= 3 {
        // output_variables[0] = FundingTargetCarrier (contains target in metadata)
        // output_variables[1] = ThresholdCheckResult 
        // output_variables[2] = ConditionalTotal
        
        // Store the conditional total for potential withdrawal
        state.balance_tracker_id = Some(output_variables[2]);
        
        // Always reveal the threshold check result (just 1 or 0)
        // But only reveal the actual total if threshold was met
        (
            state,
            vec![],
            vec![ZkStateChange::OpenVariables {
                variables: vec![output_variables[1]], // Only reveal threshold check result initially
            }],
        )
    } else {
